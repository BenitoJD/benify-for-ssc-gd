  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-4xl font-bold tracking-tight text-[var(--text-main)] mb-2">Topics</h1>
          <p className="font-bold text-[var(--text-muted)] text-sm uppercase tracking-widest">Create and manage topics with subject association</p>
        </div>
        <button
                ...
                {topics.map((topic) => (
                  <tr key={topic.id} className="hover:bg-gray-50/50 transition-colors group">
                    <td className="px-6 py-5">
                      <div>
                        <p className="font-bold text-[var(--text-main)] text-[15px]">{topic.name}</p>
                        {topic.description && (
                    ...
                    <td className="px-6 py-5 font-bold text-[var(--text-main)] text-[13px]">{getSubjectName(topic.subject_id)}</td>
                    <td className="px-6 py-5 text-center font-extrabold text-[var(--text-main)] text-xl">{topic.lesson_count}</td>
                    ...
                    <td className="px-6 py-5">
                      <select
                        ...
                        className={`pl-3 pr-8 py-1.5 text-[10px] uppercase font-bold tracking-widest rounded-lg border-2 cursor-pointer transition-colors ${
                          topic.status === 'published' ? 'bg-green-100 text-green-700 border-green-200' :
                          topic.status === 'review' ? 'bg-yellow-100 text-yellow-700 border-yellow-200' :
                    ...
                    <td className="px-6 py-5 text-center font-bold text-[var(--text-muted)] text-[15px]">{topic.order_index}</td>
                    <td className="px-6 py-5">
